...
import { Momentary, Momentarydata, Settings } from '@app/models';
...
export class RestapiService {
  ...
  momentaryValues(): Observable<any> {
    return this.http
      .get(`${environment.apiUrl}/all/all/now`)
      .pipe(map(this.extractMomentaryData));
  }

  getConfig(): Observable<Settings[]> {
    return this.http.get<Settings[]>(`${environment.apiUrl}/config/read`);
  }
}
